...
API_TOKEN = "MTAxOjxRUwDsNUj-Jmsyyg-V25AMLtt0"
FOLDER_ID = 78
FILE_LIST_PATH = r"C:\DBG_ImageProcessor\fileList.txt"
...
output_file = rf"C:\DBG_ImageProcessor\{todaysDate}_mycoportalMediaImportMapping.csv"

base_url = "https://botanicgardens.netx.net/file/asset"
rights_url = "https://creativecommons.org/publicdomain/zero/1.0/"
pattern = re.compile(r"(DBG-F-\d{6})", re.IGNORECASE)
...
with open(output_file, "w", newline="", encoding="utf-8") as csvfile:
    ...
    for item in data:
        ...
        if not match:
            raise ValueError(
                f"Filename does not contain a valid catalog number (DBG-F-######): {name}"
            )
